...
/** The Zenn Studio wordmark: a monogram tile + letter-spaced name. */
export function Logo({ className, full = true, onClick }: LogoProps) {
  return (
    <Link
      to="/"
      onClick={onClick}
      aria-label="Zenn Studio — home"
    ...
    >
      <span className="flex h-9 w-9 items-center justify-center rounded-md border border-white/15 bg-white/[0.02] transition-colors duration-500 group-hover:border-white/35">
        <span className="font-display text-sm font-bold tracking-tight text-chalk">Z</span>
      </span>
      <span className="flex flex-col leading-none">
        <span className="font-display text-sm font-semibold tracking-[0.02em] text-chalk">Zenn Studio</span>
        {full && (
